...
- The `getOwnMemory()` then accesses the DATA RAM using an object of `class Mmap` called `ownMemory`:<br>The `dataram0` and `1` (shown in the PRU-ICSS  block diagram) is `8-KiB`.
- The `getSharedMemory` function then accesses the shared RAM using an object of `class Mmap` called `sharedMemory`:<br>The general purpose memory RAM (signified RAM2) shared between PRU0 and PRU1 is `32-KiB`.
